...
def main() -> None:
    ...
    for prompt, response in zip(baseline_result.prompts, baseline_result.responses):
        print(f"\n[PROMPT] {prompt}")
        print(f"[RESPONSE] {response[:300]}")

    # Steered: subtract refusal direction
    print("\n" + "=" * 60)
    print("STEERED (project_subtract refusal direction)")
    print("=" * 60)
    ...
    for prompt, response in zip(steered_result.prompts, steered_result.responses):
        print(f"\n[PROMPT] {prompt}")
        print(f"[RESPONSE] {response[:300]}")
